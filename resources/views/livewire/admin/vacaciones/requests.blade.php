<div>
                ...
                <div class="p-6 lg:p-8 bg-white dark:bg-gray-800 dark:bg-gradient-to-bl dark:from-gray-700/50 dark:via-transparent border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg shadow">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div>
                                <label for="filtroEstadoVac" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Estado</label>
                                <select wire:model.live="filtroEstado" id="filtroEstadoVac" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                            ...
                            <div>
                                <label for="filtroTrabajadorVac" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Trabajador</label>
                                <select wire:model.live="filtroTrabajador" id="filtroTrabajadorVac" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                                    <option value="">Todos</option>
                            ...
                            <div>
                                <label for="filtroFechaDesdeVac" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Desde Fecha Inicio</label>
                                <input type="date" wire:model.live="filtroFechaDesde" id="filtroFechaDesdeVac" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200 rounded-md">
                            </div>
                            <div>
                                <label for="filtroFechaHastaVac" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Hasta Fecha Fin</label>
                                <input type="date" wire:model.live="filtroFechaHasta" id="filtroFechaHastaVac" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200 rounded-md">
                            </div>
                        </div>
                    </div>
                                        ...
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td class="py-4 px-6 whitespace-nowrap">
                                                @if($solicitud->Estado_Solicitud == 'Pendiente')
                                                    <button wire:click="abrirModalGestion({{ $solicitud->ID_Vacaciones }}, 'aprobar')" class="px-2 py-1 text-xs font-medium text-green-700 bg-green-100 rounded-md hover:bg-green-200 dark:bg-green-600 dark:text-green-100 dark:hover:bg-green-700">Aprobar</button>
                                                    <button wire:click="abrirModalGestion({{ $solicitud->ID_Vacaciones }}, 'rechazar')" class="ml-2 px-2 py-1 text-xs font-medium text-red-700 bg-red-100 rounded-md hover:bg-red-200 dark:bg-red-600 dark:text-red-100 dark:hover:bg-red-700">Rechazar</button>
                                                @else
                                                    @php
                                                        $accionRevisar = ($solicitud->Estado_Solicitud == 'Aprobada') ? 'rechazar' : 'aprobar';
                                                    @endphp
                                                    <button wire:click="abrirModalGestion({{ $solicitud->ID_Vacaciones }}, '{{ $accionRevisar }}')" class="px-2 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 dark:bg-gray-600 dark:text-gray-100 dark:hover:bg-gray-500">Revisar</button>
                                                @endif
                                                <button wire:click="$dispatch('open-detail-modal-vacaciones', { id: {{ $solicitud->ID_Vacaciones }} })" class="ml-2 px-2 py-1 text-xs font-medium text-blue-700 bg-blue-100 rounded-md hover:bg-blue-200 dark:bg-blue-600 dark:text-blue-100 dark:hover:bg-blue-700">Detalles</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                    ...
                    @else
                        <p class="text-gray-600 dark:text-gray-400">No hay solicitudes de vacaciones que coincidan con los filtros aplicados.</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div x-data="{ open: false }" @open-gestion-modal.window="open = true" @close-gestion-modal.window="open = false" x-show="open" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title-vac" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-lg p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-800 shadow-xl rounded-lg">
                <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-gray-100" id="modal-title-vac">
                ...
                <form wire:submit.prevent="gestionarSolicitud" class="mt-4 space-y-4">
                    <div>
                        <label for="comentariosAdminVac" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Comentarios del Administrador <span x-show="$wire.accionModal === 'rechazar'" class="text-red-500">* (requerido si rechaza)</span>
                        </label>
                        <textarea wire:model.defer="comentariosAdmin" id="comentariosAdminVac" rows="4" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-md focus:ring-indigo-500 focus:border-indigo-500"></textarea>
    ...
    <div x-data="{ openDetailVacaciones: false, vacacionDetail: null }" 
         @open-detail-modal-vacaciones.window="openDetailVacaciones = true; vacacionDetail = $event.detail.vacacionData;" 
        ...
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div x-show="openDetailVacaciones" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-800 shadow-xl rounded-lg">
                <div class="flex justify-between items-center">
                    <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-gray-100" id="detail-modal-title-vac">
                        Detalles de la Solicitud de Vacaciones
                    </h3>
                    <button @click="openDetailVacaciones = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                    ...
                    <div x-show="vacacionDetail">
                        <dl class="divide-y divide-gray-200 dark:divide-gray-700">
                            <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2" x-text="vacacionDetail?.trabajador_nombre"></dd>
                            </div>
                             <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Email</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2" x-text="vacacionDetail?.trabajador_email"></dd>
                            </div>
                            <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Fechas Solicitadas</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2"><span x-text="vacacionDetail?.fecha_inicio_formatted"></span> - <span x-text="vacacionDetail?.fecha_fin_formatted"></span></dd>
                            </div>
                            <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Días Laborables Solicitados</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2" x-text="vacacionDetail?.dias_solicitados"></dd>
                            </div>
                            <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" 
                                          :class="{
                                            'bg-yellow-100 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-100': vacacionDetail?.estado_solicitud === 'Pendiente',
                                            'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100': vacacionDetail?.estado_solicitud === 'Aprobada',
                                            'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100': vacacionDetail?.estado_solicitud === 'Rechazada'
                                          }"
                            ...
                            <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2" x-text="vacacionDetail?.fecha_solicitud_formatted"></dd>
                            </div>
                            <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4" x-show="vacacionDetail?.aprobado_por_nombre">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2">
                                    ...
                                    <template x-if="vacacionDetail?.fecha_aprobacion_rechazo_formatted">
                                        <span class="text-xs text-gray-500 dark:text-gray-400" x-text="' (' + vacacionDetail.fecha_aprobacion_rechazo_formatted + ')'"></span>
                                    </template>
                                </dd>
                            </div>
                             <div class="py-3 sm:grid sm:grid-cols-3 sm:gap-4" x-show="vacacionDetail?.comentarios_admin">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 sm:mt-0 sm:col-span-2 whitespace-pre-wrap" x-text="vacacionDetail?.comentarios_admin"></dd>
                            </div>
                        </dl>
                    </div>
                </div>
            ...
            Livewire.on('open-detail-modal-vacaciones', (event) => {
                if (event.id) { 
                    @this.call('getVacacionDetailsForModal', event.id).then(details => {
                        if (details) {
                            let alpineComponent = document.querySelector('[x-data*="openDetailVacaciones"]').__x;
                            alpineComponent.vacacionDetail = details;
                            alpineComponent.openDetailVacaciones = true;
                        } else {
